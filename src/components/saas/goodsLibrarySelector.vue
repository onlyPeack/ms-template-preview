<template>
    <el-dialog :visible.sync="dialogVisible" width="80%" @close="handleClose" :close-on-click-modal="false" :modal-append-to-body="false" :nowClassify="nowClassify"
               :before-close="handleClose" :title="this.openERP*1===1?'从梅施产品库导入':'从产品库导入'" append-to-body class="public-dialog">
...
<!--                            </el-col>-->
                ...
                <el-col :span="20" style='padding-left:10px;'>
                    <!-- 查询和其他操作 -->
                    <div class="filter-container" style="background: #ffffff;padding-left: 20px;">
                        <div class="classify">
                        ...
                        <el-input clearable class="filter-item" @keyup.enter.native="handleFilter" style="width: 200px;margin-right: 15px;margin-bottom: 15px;" placeholder="请输入产品品牌名称" v-model="listQuery.brandName">
                        </el-input>
...
<!--                        </el-select>-->
                    ...
                    <div class="result-container"style="padding: 0% 1%; background: #ffffff;">
                        <el-table size="small" :data="list" v-loading="listLoading" @selection-change="handlechangeFun" @row-click="rowClick" ref="list"
                            ...
                            <el-table-column label="同步商品" align="center" v-slot="{row}">
                                <i class="el-icon-refresh" style="cursor: pointer;font-size: 18px" @click="doImport(row)"></i>
                            </el-table-column>
                            ...
                            <el-table-column align="center" label="产品图片" prop="pictureName" v-slot="{row}">
                                <el-image :src="row.pictureName[0]" alt="" v-if="row.pictureName&&row.pictureName!==''"></el-image>
                            </el-table-column>
                            <el-table-column align="center" label="产品系列名称" prop="productSeries">
                            </el-table-column>
                            <el-table-column align="center" label="产品订货号" prop="itemNo" v-slot="{row}" width="130px">
                    ...
                    <!-- 分页 -->
                    <div class="pagination-container" style="padding: 10px 0px;background: #ffffff;position:relative">
                        <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
                                       :current-page="listQuery.page"
                                       :page-sizes="[10,20,30,50,1000]" :page-size="listQuery.limit"
                                       layout="total, sizes, prev, pager, next, jumper" :total="total">
                        </el-pagination>
                        <el-button type="primary" @click="doImportWithSeries" icon="el-icon-download" style="position: absolute;right: 35px;top: 10px">导出(按分类)</el-button>
                    </div>
                </el-col>
            </el-row>
        </list-container>
...
<script>
    ...
    import {coverFromOut} from '@/api/erp/saas/allGoods';
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                ...
                let method=this.openERP*1===1?selectPageFromErp:page
                ...
                method(params).then(response => {
                    ...
                    for (let i = 0; i <this.list.length ; i++) {
                        this.list[i].pictureName=eval(this.list[i].pictureName)
                    }
            ...
            doImport(row){
                let params={
                    selfCategoryId:this.nowClassify,
                    productId:row.productId,
                    categoryId:this.listQuery.categoryId,
                    categoryName:this.listQuery.categoryName
                }
                //console.log(this.nowClassify,'classfiy')
                coverFromOut(params).then(res=>{
            ...
            doImportWithSeries(){
                if(this.listQuery.categoryId&&this.listQuery.categoryId!==''){
                    let params={
                        selfCategoryId:this.nowClassify,
                        categoryId:this.listQuery.categoryId,
                        categoryName:this.listQuery.categoryName
